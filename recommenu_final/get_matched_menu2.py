    ...
    while(first_true):
        ...
        while(sec_true):
            try:
                ...
                menu_id = js['response']['menu']['menus']['items'][0]['menuId']
                # should add an array with the vaues
                fs_menu[item_name.lower()] = [fs_id, menu_id, item_id]
                fs_count += 1
                sec_count += 1
            except:
                sec_true = False
...
def yelp_match(fs_menu, yelpurl):
    ...
    writefile = open("item_ratings_boston_new3.txt", "a")
    for name, url in yelp_menu.items():
        #if try works, then can get the ratings for it.
        try:
            fsid = fs_menu[name][0]
            menu_id = fs_menu[name][1]
            item_id = fs_menu[name][2]
            #print 'yes'
            #now go get ratings
            rating_list = get_ratings(url)  #returns [likes, dislikes]
            yelp_count -= 1
            print 'Writing...', yelp_count
            '''

            Need to come up with a way to say ratings one at a time
            good = 1
            bad = 0
            ...
            countt = 0
            #in rating list - have 2 ratings. 
            for item in rating_list:
                print item
                for x in range(0, int(item)):
                    if countt == 0:
                        #print '1'
                        writefile.write(' ,1, ,'+fsid+","+menu_id+","+item_id+"\n")
                    else:
                        #print '0'
                        writefile.write(' ,0, ,'+fsid+","+menu_id+","+item_id+"\n")
                    #first round is positive, second is negative
                #print item
            ...
            print 'Here is an error: Help Me!'
            pass
...
def main():
    ...
    fs_count = 0
    infile= open("boston_fsid_final_chains2.txt", "r")
    for line in infile:
        #print line
        line = line.strip()
        #print line
    ...
    '''
    infile = open("fsname_yelpurl_boston_2.txt", "r")
    yelp = {}
    for line in infile:
        line = line.strip()
        line = line.split(",")
        restname = line[0].lower().strip()
        menurl = line[1].strip()
        #print restname
    ...
    outfile = open("track_of_rest_match_boston3.txt","a")
    outfile.close()
    outfile = open("track_of_rest_match_boston3.txt","r")
    rest_exist = {}
    for x in outfile:
        x = x.split(",")
        x = x[0].strip()
